...
#define DMA_TRANSFER_MATMUL_SIZE 4096*8

////////////////////////////////////////////////////////////////////////
////////////////      OTHER  DEFINES      /////////////////////////////
////////////////////////////////////////////////////////////////////////

#define PRINT_ENTRY_MATRICES 0
#define PRINT_OUTPUT_MATRICES 1
...
int main(void) {
    ...
    uint32_t resultado[DMA_TRANSFER_MATMUL_SIZE];   

    for (size_t j = 0; j < 8; j++) {
        for (size_t i = 0; i < DMA_TRANSFER_MATMUL_SIZE /8; i++) {
            src[j*(DMA_TRANSFER_MATMUL_SIZE /8) + i] = ((j+1)%2)*2 + j; 
        }
    }

    if(PRINT_ENTRY_MATRICES){
        //Para imprimir las matrices de entrada
        print_uart("MATRICES DE ENTRADA \n\n\r");
        for (int i=0; i<8; i++){
            print_uart("\r\n MATRIZ "); print_uart_int(i); print_uart("\r\n");
            for (int j=0; j<64; j++){
                for (int k=0; k<64; k++){
                    print_uart_int(src[i*4096 + j*64 + k]);
                    print_uart("  ");
                }
                print_uart("\r\n");
            }
    ...
    *shuffler_a3_id_reg_low  = 0x1111;
    *shuffler_a3_block_size_reg  = 4096*2; 
    *shuffler_a3_clock_gate_reg  = 0b1111; 
    ...
    //Sending data to Artico3
    setup_transfer((uint64_t)&src, (uint64_t)artico_data, (uint32_t)DMA_TRANSFER_MATMUL_SIZE*4, 1, 1, &desc);
    submit_transfer(&desc);
    wait_for_transfer(&desc);

    //Waiting for Artico3 to finish the matrix multiplication
    do {
        print_uart("Ready register: ");
        print_uart_int(*shuffler_a3_ready_reg);
        print_uart("\r\n");
        
    } while (*shuffler_a3_ready_reg != 0xf);

    // FLUSH_D_CACHE();

    //Reading the results from Artico3
    setup_transfer((uint64_t)matmul, (uint64_t)&resultado, (uint32_t)DMA_TRANSFER_MATMUL_SIZE*4, 1, 1, &desc);
    submit_transfer(&desc);
    wait_for_transfer(&desc);


    if (PRINT_OUTPUT_MATRICES){
        print_uart("\n\n\rResultados obtenidos !! \n\n\r");
        for (int k=0; k<8; k++){
            if (k%2 == 0){
                for (int i=0; i<64; i++){
                    for (int j=0; j<64; j++){
                        print_uart_int(resultado[k*4096 + i*64 + j]);
                        print_uart("  ");             
                    }
                    print_uart("\r\n");
                }
