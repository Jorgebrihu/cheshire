...
void setup_transfer(uint64_t src, uint64_t dst, uint32_t len, bool do_irq, bool decouple_rw,
    ...
    desc->flags = 0xff << 16 | (1 << 6) | (decouple_rw << 5) | (0x1 << 1) | do_irq;
}
...
int main(void) {
    ...
    uint64_t src[300];
    uint64_t dst[300];
    volatile uint64_t *myram = (volatile uint64_t *) MYRAM;
    ...
    print_uart("\n\rDireccion de myram: "); print_uart_int((uint32_t)myram);

    print_uart("\n\rSize of src: ");
    print_uart_int_64(sizeof(src));

    // for (int i = 0; i<8; i++){
    //     print_uart("\n\rValue of myram["); print_uart_int((uint32_t)&myram[i]); print_uart("]: ");
    //     print_uart_int_64(*(myram+i));
    // }
    ...
    setup_transfer((uint64_t)&src, (uint64_t)myram, (uint32_t)sizeof(src), 1, 1, &desc);
