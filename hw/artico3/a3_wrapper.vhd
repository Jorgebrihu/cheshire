...
entity a3_wrapper is
    generic (
        ...
        C_MAX_MEM            : integer := 49152; -- Total memory size (in bytes) inside the compute unit
        C_NUM_MEM            : integer := 3;     -- Number of memory banks inside the compute unit (to allow parallel accesses from logic
        C_NUM_REG            : integer := 0      -- Number of Read/Write registers inside the compute unit
    );
    port (
...
architecture behavioral of a3_wrapper is
    ...
    -- signal int_to_std : std_logic_vector(31 downto 0); -- 32-bit std_logic_vector to match the integer size

    -- component xlnx_ila_wrapper
    --     port (
    --         clk    : in std_logic;
    ...
    -- attribute mark_debug of bram_0_WEN_A  : signal is "TRUE";
    -- attribute mark_debug of bram_0_Addr_A : signal is "TRUE";

    -- attribute mark_debug of bram_1_WEN_A  : signal is "TRUE";
    -- attribute mark_debug of bram_1_Addr_A : signal is "TRUE";

    -- attribute mark_debug of bram_2_WEN_A  : signal is "TRUE";
    -- attribute mark_debug of bram_2_Addr_A : signal is "TRUE";
...
begin
    ...
    -- -----------------------------
    -- -- ILA wrapper Instantiate --
    -- -----------------------------

    -- int_to_std <= std_logic_vector(to_unsigned(data_cnt, 32)); -- Convert integer to 32-bit std_logic_vector

    -- ila_wrapper : xlnx_ila_wrapper
    --     port map (
    --         clk    => s_artico3_aclk,          
    --         probe0 => mem_out(0),         
    --         probe1 => mem_out(1),  
    ...
    port map (
        ...
        bram_0_Din_A   => mem_din(0),
        bram_0_Dout_A  => mem_dout(0),
        ...
        bram_1_Din_A   => mem_din(1),
        bram_1_Dout_A  => mem_dout(1),
        ...
        bram_2_Din_A   => mem_din(2),
        bram_2_Dout_A  => mem_dout(2),

        values         => std_logic_vector(to_unsigned(data_cnt, 32))
    );

    -- Adapt HLS memory signals
    mem_we(0)   <= bram_0_WEN_A(0) or bram_0_WEN_A(1) or bram_0_WEN_A(2) or bram_0_WEN_A(3);
    mem_addr(0) <= std_logic_vector(resize(shift_right(unsigned(bram_0_Addr_A), 2), C_ARTICO3_ADDR_WIDTH));

    mem_we(1)   <= bram_1_WEN_A(0) or bram_1_WEN_A(1) or bram_1_WEN_A(2) or bram_1_WEN_A(3);
    mem_addr(1) <= std_logic_vector(resize(shift_right(unsigned(bram_1_Addr_A), 2), C_ARTICO3_ADDR_WIDTH));

    mem_we(2)   <= bram_2_WEN_A(0) or bram_2_WEN_A(1) or bram_2_WEN_A(2) or bram_2_WEN_A(3);
    mem_addr(2) <= std_logic_vector(resize(shift_right(unsigned(bram_2_Addr_A), 2), C_ARTICO3_ADDR_WIDTH));
